...
def validate_duration_ms(duration_ms):
    ...
    if (duration_ms is None):
        raise Exception("Request params (duration_ms) not found")

    # if duration params is empty
    if not duration_ms: 
        raise Exception("duration_ms is empty")

    # check if duration is integer
    if isinstance(duration_ms, int):
        raise Exception("duration_ms needs to be integer")

    # ensure duration is more than 0
    if int(duration_ms) <= 0:
        raise Exception("duration_ms must be more than zero")
